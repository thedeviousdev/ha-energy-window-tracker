...
def _parse_windows(config: dict[str, Any]) -> list[WindowConfig]:
    ...
    _MAIN_LOGGER.info("_parse_windows: len(windows_data)=%s", len(windows_data))
    windows = []
...
class WindowData:
        ...
        if stored:
            ...
            for w in self._windows:
                if str(w.index) in snapshots_data:
                    ...
                    loaded += 1
            _MAIN_LOGGER.info("sensor: load - %s snapshot_date=%s loaded %s window(s)", self._source_entity, self._snapshot_date, loaded)
        else:
            _MAIN_LOGGER.info("sensor: load - %s no stored data", self._source_entity)
    ...
    async def save(self) -> None:
        ...
        _MAIN_LOGGER.info("sensor: save - %s snapshot_date=%s %s window(s)", self._source_entity, self._snapshot_date, len(snapshots_data))
    ...
    def _handle_window_start(self, window: WindowConfig, now: datetime) -> None:
        ...
        if value is not None:
            ...
            _MAIN_LOGGER.info("sensor: window '%s' start - %.3f kWh", window.name, value)
            self._schedule_save()
        self._notify_update()
    ...
    def _handle_window_end(self, window: WindowConfig, now: datetime) -> None:
        ...
        if value is not None:
            ...
            _MAIN_LOGGER.info("sensor: window '%s' end - %.3f kWh", window.name, value)
            self._schedule_save()
        self._notify_update()

    def _handle_midnight(self, now: datetime) -> None:
        """Reset snapshots at midnight (day always starts at 00:00)."""
        _MAIN_LOGGER.info("sensor: _handle_midnight - resetting snapshots for %s", self._source_entity)
        self._snapshots = {
            w.index: WindowSnapshots(snapshot_start=None, snapshot_end=None)
            for w in self._windows
        }
...
async def async_setup_entry(
    ...
    _MAIN_LOGGER.info("sensor: async_setup_entry - entry_id=%s, setting up entities", entry.entry_id)
    config = {**entry.data, **entry.options}
    sources = _get_sources_from_config(config)
    if not sources:
        _MAIN_LOGGER.info("sensor: async_setup_entry - no sources in config")
        return
    ...
    for source_index, source_config in enumerate(sources):
        if not isinstance(source_config, dict):
            _MAIN_LOGGER.info("sensor: async_setup_entry - source %s is not a dict", source_index)
            continue
        source_entity = source_config.get(CONF_SOURCE_ENTITY)
        if not source_entity:
            _MAIN_LOGGER.info("sensor: async_setup_entry - source %s has no source_entity", source_index)
            continue
...
class WindowEnergySensor(RestoreSensor):
    ...
    async def async_added_to_hass(self) -> None:
        ...
        _MAIN_LOGGER.info("sensor: added to hass - %r entity_id=%s", self._window_name, self.entity_id)
