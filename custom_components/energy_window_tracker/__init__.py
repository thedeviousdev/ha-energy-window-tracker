...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    _MAIN_LOGGER.debug("init: async_unload_entry - entry_id=%s ok=%s", entry.entry_id, unload_ok)
    return unload_ok


async def async_update_options(hass: HomeAssistant, entry: ConfigEntry) -> None:
    """Update options."""
    _MAIN_LOGGER.debug("init: async_update_options - entry_id=%s, reloading", entry.entry_id)
